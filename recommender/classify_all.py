...
def save_predict_results(prediction, s_params, model_name, hier_name):
    ...
    result.append("Time finished: " + datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
    result.append("AUPCR: " + str(average_precision_score(test_labels, prediction)))

    k_values = [1, 2, 3, 4, 5, 7, 10]

    r, p, f = prf_at_k(test_labels, prediction, k_values)

    result.append(", ".join(["P@" + str(k) + ": " + p["P@" + str(k)]for k in k_values]))
    result.append(", ".join(["R@" + str(k) + ": " + r["R@" + str(k)]for k in k_values]))
    result.append(", ".join(["F@" + str(k) + ": " + f["F@" + str(k)]for k in k_values]))
