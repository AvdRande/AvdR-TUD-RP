...
def train(train_feature_vector, train_labels, hierarchy, label_names, epochs):
    ...
    for level in range(depth):
        ...
        feature_matrix = train_feature_vector if level == 0 else np.concatenate(
            (predictions[level - 1], train_feature_vector), axis=1)
        ...
        target_labels = [target_labels_at_level(binlabels_to_text(
            label, label_names), hierarchy, level) for label in train_labels]
...
def predict(model, test_feature_vector, depth):
    ...
    intermediate_predictions.append(
        model[0].predict_proba(test_feature_vector))
    for i in range(1, depth):
        intermediate_predictions.append(model[i].predict(np.concatenate(
            (intermediate_predictions[-1], test_feature_vector), axis=1)))
