...
def train_model(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), [target[0].to(device), target[1].to(
            device), target[2].to(device), target[3].to(device), target[4].to(device)]
        optimizer.zero_grad()
...
class Net(nn.Module):
    ...
    def forward(self, x, training=True):
        Ag1 = F.dropout(self.batch_norm1(F.relu(self.global1(x))),
                        p=self.dropout, training=training)
        Al1 = F.dropout(self.batch_normL1(F.relu(self.local1(Ag1))),
                        p=self.dropout, training=training)
        Pl1 = torch.sigmoid(self.localOut1(Al1))

        Ag2 = F.dropout(self.batch_norm2(F.relu(self.global2(
            torch.cat([Ag1, x], dim=1)))), p=self.dropout, training=training)
        Al2 = F.dropout(self.batch_normL2(F.relu(self.local2(Ag2))),
                        p=self.dropout, training=training)
        Pl2 = torch.sigmoid(self.localOut2(Al2))

        Ag3 = F.dropout(self.batch_norm3(F.relu(self.global3(
            torch.cat([Ag2, x], dim=1)))), p=self.dropout, training=training)
        Al3 = F.dropout(self.batch_normL3(F.relu(self.local3(Ag3))),
                        p=self.dropout, training=training)
        Pl3 = torch.sigmoid(self.localOut3(Al3))

        Ag4 = F.dropout(self.batch_norm4(F.relu(self.global4(
            torch.cat([Ag3, x], dim=1)))), p=self.dropout, training=training)
        Al4 = F.dropout(self.batch_normL4(F.relu(self.local4(Ag4))),
                        p=self.dropout, training=training)
...
def train(train_feature_vector, train_labels, hierarchy, label_names, epochs):
    ...
    labels_at_lvls = [
        [target_labels_at_level(binlabels_to_text(
            label, label_names), hierarchy, i) for label in train_labels]
        for i in range(4)]

    training_set = Dataset(
        train_feature_vector,
        [labels_at_lvls[3][i] + labels_at_lvls[2][i] + labels_at_lvls[1]
            [i] + labels_at_lvls[0][i] for i in range(len(labels_at_lvls[0]))],
        labels_at_lvls[0],
        labels_at_lvls[1],
        labels_at_lvls[2],
        labels_at_lvls[3]
...
def predict(model, test_feature_vector, depth):
    ...
    test_feature_tensor = torch.from_numpy(
        test_feature_vector).type(torch.FloatTensor)

    full_pred, predat0, predat1, predat2, predat3 = model.forward(
        test_feature_tensor, training=False)
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            predictions.append((beta*(torch.from_numpy(np.concatenate(
                (Pl4[0], Pl3[0], Pl2[0], Pl1[0])))) + (1 - beta)*Pg[0]).tolist()[:220])
